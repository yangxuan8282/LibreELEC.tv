...
elif [ "$UBOOT_VERSION" = "khadas" ]; then
  PKG_VERSION="2a56c9f"
  PKG_SITE="https://github.com/khadas/u-boot"
...
make_target() {
  ...
  for UBOOT_TARGET in $UBOOT_CONFIG; do
    if [ "$PROJECT" = "Odroid_C2" ]; then
      ...
      make ARCH=arm CROSS_COMPILE=aarch64-linux-gnu- $UBOOT_TARGET
      make ARCH=arm CROSS_COMPILE=aarch64-linux-gnu-
    elif [ "$PROJECT" = "Khadas_Vim" ]; then
      export PATH=$ROOT/$TOOLCHAIN/lib/gcc-linaro-aarch64-none-elf/bin/:$PATH
      make ARCH=arm CROSS_COMPILE=aarch64-none-elf- distclean
      make ARCH=arm CROSS_COMPILE=aarch64-none-elf- $UBOOT_TARGET
...
makeinstall_target() {
  ...
  case $PROJECT in
    Khadas_Vim)
      cp -PRv $PKG_DIR/scripts/update-kvim.sh $INSTALL/usr/share/bootloader/update.sh
      #cp -PRv $ROOT/$PKG_BUILD/fip/u-boot.bin $INSTALL/usr/share/bootloader/u-boot
      cp -PRv $ROOT/$PKG_BUILD/fip/u-boot.bin.sd.bin $INSTALL/usr/share/bootloader/u-boot
      ;;
    Odroid_C2)
